...
log_sigmas_10 = torch.logspace(start=torch.log10(torch.tensor(0.01)),
                            ...
                            steps=15)
log_sigmas_120 = torch.logspace(start=torch.log10(torch.tensor(0.01)),
                            end=torch.log10(torch.tensor(200.0)),
                            steps=30)
...
sns.heatmap(scores_tensor.cpu().numpy(), annot=True, cmap="viridis", xticklabels=[f"{x:.2f}" for x in X.cpu().tolist()], yticklabels=Y.cpu().tolist())

plt.xlabel('sigma_color')
plt.ylabel('sigma_space')
plt.title('Heatmap of Sigma Optimization for Specific Attack')

plt.savefig(os.path.join(output_dir, "hyperparams_large_optimization_specific.png"))
plt.close()