...
def save_batch_denoised_figures(images_and_labels, batch_index, model, attacker, sig):
    ...
    for i, (image, labels) in enumerate(images_and_labels):
        ...
        adv_img_specific, _ = attacker.eot_attack_specific(image_input, target, num_iter=num_iter, sigma_values =sigmas, n_monte_carlo = 30)
    ...
    return batch_ground_truth_preds, batch__denoised_ground_truth_preds, batch_specific_attack_preds, batch__denoised_specific_attack_preds
...
for sig in sigmas:
    ...
    for batch_index in range(0, n_samples, batch_size):
        ...
        ground_truth_preds, denoised_ground_truth_preds, specific_preds, denoised_specific_preds = save_batch_denoised_figures(
        ...
        for gt_pred, dns_gt_pred, spec_pred, dns_spec_pred, label in zip(ground_truth_preds, denoised_ground_truth_preds, specific_preds, denoised_specific_preds, labels):
    ...
    print(f"Sigma: {sig}, EOT Specific Attack Success Rate: {specific_attack_success_rate:.2f}%")
    print(f"Sigma: {sig}, Denoised EOT Specific Attack Success Rate: {denoised_specific_attack_success_rate:.2f}%")
    print(f"Sigma: {sig}, Denoised EOT Specific Attack Right Label Rate: {denoised_right_label_rate:.2f}%")
    print(f"Sigma: {sig}, Consistency Success Rate: {consistency_success_rate:.2f}%")

# Plot success rate vs sigma
plt.figure(figsize=(8, 6))
plt.plot(sigmas, specific_attack_success_rates, label="EOT Specific Attack", marker='o', color='blue')
plt.plot(sigmas, denoised_specific_attack_success_rates, label="Denoised EOT Specific Attack", marker='o', color='green')
plt.plot(sigmas, consistency_success_rates, label='Consistency without attack', marker='o', color='yellow')
plt.plot(sigmas, denoised_right_label_rates, label='Denoised EOT Specific Attack Right Label Rate', marker='o', color='red')
plt.title("Adversarial Attack Success Rate vs Sigma")
plt.xlabel("Sigma")
plt.ylabel("Success Rate (%)")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(output_dir, "eot_attack_success_vs_sigma.png"))
plt.close()